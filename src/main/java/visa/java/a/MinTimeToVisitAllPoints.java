...
public class MinTimeToVisitAllPoints {
    // O(1) Time, O(1) Space: Calculate Chebyshev distance between two points
    public static int timeToVisit(int[] src, int[] dst) {
        return Math.max(
    ...
    // Overall Runtime: O(n) Time, O(1) Space
    public int minTimeToVisitAllPoints(int[][] points) {
        // Edge case: Return 0 time for null or empty points array
        ...
        // O(n) Time: Iterate through points and accumulate distances
        for (int i = 1; i < points.length; i++) {
            int[] next = points[i];
            time += timeToVisit(curr, next);
            curr = next;
        }
        return time;
    }
