 ...
 * The decrypted code is [7+1+4, 1+4+5, 4+5+7, 5+7+1]. Notice that the numbers
 * wrap around.
 * 
 * Example 2:
 ...
 * - -(n - 1) <= k <= n - 1
 */
public class CircularArrayDefuse {
    ...
    public int[] decrypt(int[] code, int k) {
        ...
        int currentSum = 0;
        // O(k) Time: Calculate the initial window sum
        for (int i = start; i <= end; i++) {
            currentSum += code[i % n];
        }
        
        // O(n) Time: Slide the window across the circular array
        for (int i = 0; i < n; i++) {
            result[i] = currentSum;
            // Subtract the outgoing element and add the incoming element
            currentSum -= code[(start + i) % n];
            currentSum += code[(end + i + 1) % n];
        }
