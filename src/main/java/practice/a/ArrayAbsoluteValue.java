package practice.a;

/**
 * Array Absolute Value Challenge
 * 
 * Given an array of integers 'nums', produce an array of integers composed of
 * the absolute values of all the numbers in 'nums'. Absolute value represents
 * the distance from zero, so negative numbers should become positive and
 * positive numbers should stay the same.
 */
public class ArrayAbsoluteValue {
    /**
     * @param nums an array.
     * @return the absolute value array of nums.
     */
    public int[] getArrayAbs(int[] nums) {
        int[] numsAbs = new int[nums.length];
        for (int i = 0; i < nums.length; i++) {
            numsAbs[i] = (nums[i] < 0) ? (-nums[i]) : (nums[i]);
        }
        return numsAbs;
    }
}
